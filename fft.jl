...
function fft_(coefs::Vector{Int64}, ws::Vector{Int64})
    if length(coefs) == 1
        return Vector{Int}([coefs[1]])
    end
    even_ixs = 2*(1:div(length(coefs), 2))
    ws2 = ws[even_ixs.-1] # squares of roots of unity
    even = fft_(coefs[even_ixs], ws2)
    odd = fft_(coefs[even_ixs.-1], ws2)
    append!(odd, odd)
    append!(even, even)
    (odd + (ws .* even) .% F) .% F
end
...
function dump(coefs::Vector{Int64}, ws::Vector{Int64})
...
ffted = fft_(fft_coefs_, ws)
fft_inverse = fft_(ffted, iws) .* powermod(N, F-2, F) .% F
